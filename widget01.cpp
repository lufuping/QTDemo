    ...
    , ui(new Ui::Widget01)
{
    ...
    menu_ui=new Menu();
    dPlayer_ui=new Player();
    dMusicPlayer_ui=new MusicPlayer();
    init();
    // 建立回退信号的连接
    ...
    // 建立打开音乐播放器的连接
    connect(v_ui,&ViewingFilm::openMusicPlayer,this,&Widget01::openMusicPlayer);
#if 0
    // 创建一个按钮，按下按钮窗口关闭
...
void Widget01::openMusicPlayer()
{
    dMusicPlayer_ui->show();
}
